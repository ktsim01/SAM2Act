...
DATASET_PREFIX=/home/ktsim/Projects/SAM2Act/sam2act/data_articubot/insert_onto_square_peg_orientation_discretized

echo ${DATASET_PREFIX}/${TASK,,}/
echo _${TASK}
cd third_party/robogen/
torchrun --standalone --nproc_per_node=$NP train_ddp_weighted_displacement_orientation_discretized.py \
    --batch_size $BS \
    --num_epochs $EPOCHS \
    --model_type GripperOrientNet --model_invariant \
    --exp_path $EXP_PATH \
