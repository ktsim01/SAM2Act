...
def angle_to_bin(angle_deg, num_bins=72, range_min=-180.0, range_max=180.0):
    ...
    # Clamp just in case (avoid index 72)
    return torch.clamp(bin_idx, 0, num_bins - 1)
...
def train(args):
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, data in enumerate(tqdm(train_dataloader)):
            ...
            orient_loss = (orient_loss_roll + orient_loss_pitch + orient_loss_yaw) / 3.0
            ...
            if (i+1) % 10 == 0 and os.environ['LOCAL_RANK'] == '0':
                ...
                log_info = {
                    ...
                    "orientation_loss": orient_loss.item() / 1000,
        ...
        if (epoch + 1) % 5 == 0:
            ...
            for i, data in enumerate(tqdm(val_dataloader)):
                ...
                roll_acc += (roll_pred == roll_gt).sum().float().item()
                pitch_acc += (pitch_pred == pitch_gt).sum().float().item()
                yaw_acc += (yaw_pred == yaw_gt).sum().float().item()
