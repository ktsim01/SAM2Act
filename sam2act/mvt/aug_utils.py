...
def rand_dist_same(size, min=-1.0, max=1.0):
    # Generate a single random number of shape (1, x)
    rand_value = (max - min) * torch.rand(1, size[1]) + min
    # Repeat it for the batch size dimension
    return rand_value.repeat(size[0], 1)
